...
## Section 5: company research preview

- Version 0.5.0 adds explicit research opt-in in the popup, including a data-sharing explanation, optional loopback-service permission, automatic lookup for unclear topics, and retry/offline states. Pause and site-disable controls also stop new research.
- A local TypeScript service retrieves matched official vacancies and official policy pages, with optional Brave Search discovery. Provider credentials are server-only. The default verified employer registry contains only Atlassian; unknown or ambiguous identities are not substituted with similarly named entities.
- Matching uses exact reviewed employer names/aliases, role title, and role identifier or source address. Different vacancies are not promoted to company policies. Search snippets are not cited as evidence; the source page must be fetched.
- Research is shown separately in the popup and evidence panel with quotations, retrieval dates, publication-date availability, location, and applicability scope. Historical petition activity never updates the page's sponsorship/CPT/OPT findings or badge. Imported history retains its original retrieval date and requires a reviewed legal-entity/location match; no historical dataset is preloaded.
...
Section 5 validation: **147 automated tests pass**, including 15 research tests. TypeScript checks, the rebuilt 0.5.0 extension, the existing browser suite, the real worker-to-local-service integration test, and the actual 400-pixel toolbar-popup test pass. The research source display was visually inspected. The public Atlassian careers page was successfully fetched using the production HTTPS/DNS-validation path. Search-provider integration is implemented against the official Brave API documentation but has not been exercised with a paid key; historical-data tests use fictitious fixtures, not claims about real employers.
